...
no_avl* avl_pesquisa_impl(no_avl *no, const char *id);
no_avl* avl_insere_impl(no_avl *no, const char* id, const char *str, float rating);
no_avl* avl_remove_impl(no_avl *no, const char* id, const char *str, float rating);
...
void avl_apaga(arvore_avl* avl)
{
    ...
    while(avl->raiz != NULL)
    {
        avl->raiz = avl_remove_impl(avl->raiz, avl->raiz->id, avl->raiz->str, avl->raiz->rating);
    }

    free(avl);
}

no_avl* avl_pesquisa(arvore_avl* avl, const char *id)
{
    if(avl == NULL || id == NULL)
        return NULL;

    return avl_pesquisa_impl(avl->raiz, id);
}

int avl_insere(arvore_avl* avl,const char* id, const char *str, float rating)
{
    if(avl == NULL || id==NULL || str == NULL)
        return -1;

    avl->raiz = avl_insere_impl(avl->raiz, id, str, rating);
    return 0;
}

int avl_remove(arvore_avl* avl, const char *id, const char* str, float rating)
{
    if(avl == NULL || str == NULL)
        return -1;

    avl->raiz = avl_remove_impl(avl->raiz, id, str, rating);
...
const char *avl_min(arvore_avl* avl)
{
    ...
    return avl_no_valormin(avl->raiz)->id;
}
...
const char *avl_max(arvore_avl* avl)
{
    ...
    return avl_no_valormax(avl->raiz)->id;
}
...
no_avl* avl_novo_no(const char *id, const char *str, float rating)
{
    no_avl *no = (no_avl*) malloc(sizeof(no_avl));
    no->id=(char*)malloc(sizeof(char)*strlen(str)+1);
    strcpy(no->id, id);
    no->str = (char*) malloc(sizeof(char)*(strlen(str)+1));
    strcpy(no->str, str);
    // para no->rating nao e necessario alocar memoria, porque e um float
    no->esquerda = NULL;
    no->direita  = NULL;
...
no_avl* roda_direita(no_avl *y)
{
    ...
    /* atualiza alturas */
    y->altura = max_avl(avl_altura(y->esquerda), avl_altura(y->direita))+1;
    x->altura = max_avl(avl_altura(x->esquerda), avl_altura(x->direita))+1;
...
int calc_balanceamento_avl(no_avl *N)
{
    if (N == NULL)
        return 0;
...
no_avl* avl_pesquisa_impl(no_avl* no, const char *id)
{
    ...
    if(strcmp(id, no->id) < 0)
        return avl_pesquisa_impl(no->esquerda, id);

    else if(strcmp(id, no->id) > 0)
        return avl_pesquisa_impl(no->direita, id);
...
no_avl* avl_insere_impl(no_avl *no, const char *id, const char* str, float rating)
{
    ...
    if (no == NULL)
        return avl_novo_no(id, str, rating);

    if (strcmp(id, no->id) < 0)
        no->esquerda  = avl_insere_impl(no->esquerda, id, str, rating);
    else if(strcmp(id, no->id) > 0)
        no->direita = avl_insere_impl(no->direita, id, str, rating);
    else {
        return no;
    }

    /* 2. atualiza a altura deste no ancestral */
    no->altura = max_avl(avl_altura(no->esquerda), avl_altura(no->direita)) + 1;

    /* 3. calcula o fator de balanceamento deste no ancestral para verificar
       se deixou de estar balanceado */
    int balance = calc_balanceamento_avl(no);

    /* se o no deixou de estar balanceado, existem 4 casos */

    if (balance > 1) {
        /* Arvore e' right-heavy */
    	if (calc_balanceamento_avl(no->direita) < 0) {
        ...
        /* Arvore e' left-heavy */
    	if (calc_balanceamento_avl(no->esquerda) > 0) {
...
no_avl* avl_remove_impl(no_avl* no, const char* id, const char *str, float rating)
{
    ...
    if ( strcmp(id, no->id) < 0 )
        no->esquerda = avl_remove_impl(no->esquerda,id, str, rating);

    /* se a str a ser removida é maior do que a str da raiz,
       entao esta' na sub-arvore direita */
    else if( strcmp(id, no->id) > 0 )
        no->direita = avl_remove_impl(no->direita,id, str, rating);
    ...
    else
    {
        ...
        if( (no->esquerda == NULL) || (no->direita == NULL) )
        {
            ...
            else /* caso de um filho */
            {
                ...
                no->id=realloc(no->id, (strlen(temp->id)+1)*sizeof(char));
                strcpy(no->id, id);
        ...
        else
        {
            ...
            no->id = realloc(no->id, (strlen(temp->id)+1)*sizeof(char));
            strcpy(no->id, temp->id);
            no->str = realloc(no->str, (strlen(temp->str)+1)*sizeof(char));
            strcpy(no->str, temp->str);

            /* apaga o sucessor em-ordem */
            no->direita = avl_remove_impl(no->direita, temp->id, temp->str, temp->rating);
        }
    }
